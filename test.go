...
func tampil(pesan string) response {
	...
	for dataTask.Next() {
		var tsk = task{}
		var err = dataTask.Scan(&tsk.Id, &tsk.Task, &tsk.Assignee, &tsk.Deadline)
		if err != nil {
			return response{
				Status: false,
...
func tambah(Id, Task string, Assignee string, Deadline string) response {
	...
	defer db.Close()
	_, err = db.Exec("insert into task values (?,?,?,?)", Id, Task, Assignee, Deadline)
	if err != nil {
		return response{
			Status: false,
...
func kontroler(w http.ResponseWriter, r *http.Request) {
	var tampilHtml, errTampil = template.ParseFiles("template/tampil.html")
	if errTampil != nil {
		fmt.Println(errTampil.Error())
		return
	}
	var tambahHtml, errTambah = template.ParseFiles("template/tambah.html")
	if errTambah != nil {
		fmt.Println(errTambah.Error())
		return
	}
	var ubahHtml, errUbah = template.ParseFiles("template/ubah.html")
	if errUbah != nil {
		fmt.Println(errUbah.Error())
		return
	}
	var hapusHtml, errHapus = template.ParseFiles("template/hapus.html")
	if errHapus != nil {
